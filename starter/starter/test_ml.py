...
def input_params_path(tmpdir_factory):
    ...
    params = {"seed": 909, "data": {"test_size": 0.2, "target": "salary"}}
...
def test_equal_rows_post_processing(input_data_path, output_path, input_params_path):
    train_output_processed, train_output_clean, test_output_clean = PrepData(
        input_data_path, output_path, input_params_path
...
def test_saving_data(input_data_path, output_path, input_params_path):
    ...
    assert (
        os.path.exists(testing_dir + "/train_processed_census.csv")
        and os.path.exists(testing_dir + "/train_clean_census.csv")
        and os.path.exists(testing_dir + "/test_clean_census.csv")
    )
