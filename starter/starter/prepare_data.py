...
class PrepData:
    ...
    def __init__(self, input_path, output_path, params_path, pipe = data_pipeline()):
        ...
        self.data_input = pd.read_csv(input_path).dropna(subset = [self.target])
    ...
    def preprocess(self):
        ...
        X_train_output_processed = pd.DataFrame(fitted_pipeline.transform(self.X_train).toarray())
        train_output_processed = pd.concat([X_train_output_processed, self.y_train], axis=1)

        logger.info("Saving data outputs")

        train_output_processed.to_csv(self.output_path + '/train_processed_census.csv')
        train_output_clean.to_csv(self.output_path + '/train_clean_census.csv')
        test_output_clean.to_csv(self.output_path + '/test_clean_census.csv')
        dump(fitted_pipeline, self.output_path + '/fitted_data_pipeline.joblib')
