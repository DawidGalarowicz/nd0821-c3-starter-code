...
class PrepData:
    ...
    def __init__(self, input_path, output_path, params_path, pipe = data_pipeline()):
        ...
        self.data_input = pd.read_csv(input_path, index_col=[0]).dropna(subset = [self.target])
    ...
    def preprocess(self, save = True):
        ...
        X_train_output_processed = pd.DataFrame(fitted_pipeline.transform(self.X_train), index=self.X_train.index)
        train_output_processed = pd.concat([X_train_output_processed, self.y_train], axis=1)

        logger.info("Saving data outputs")

        if save:
            train_output_processed.to_csv(os.path.join(self.output_path, "train_processed_census.csv"))
            train_output_clean.to_csv(os.path.join(self.output_path, "train_clean_census.csv"))
            test_output_clean.to_csv(os.path.join(self.output_path, "test_clean_census.csv"))
            dump(fitted_pipeline, os.path.join(self.output_path, "fitted_data_pipeline.joblib"))
    ...
    def _data_splitting(self):
        ...
        self.data_input.loc[:, self.target] = self.data_input[self.target].map(label_mapping)

        X_train, X_test, y_train, y_test = train_test_split(
                                        self.data_input.drop(columns=[self.target]), 
                                        self.data_input[[self.target]], 
                                        random_state=self.params['seed'], test_size=self.params['data']['test_size'])
